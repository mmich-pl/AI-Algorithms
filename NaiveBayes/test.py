...
if __name__ == "__main__":
	...
	print("Train Accuracy: {}".format(
	    accuracy_score(y_train, nb_clf.predict(X_train))))
	print("Test Accuracy: {}".format(accuracy_score(y_test, nb_clf.predict(X_test))))

	#Query 1:
	query = np.array([['deszcz', 'tak', 'niska', 'niska']])
	print("Query 1:- {} ---> {}".format(query, nb_clf.predict(query)))

	#Query 2:
	query = np.array([['zachmurzenie', 'tak', 'niska', 'srednia']])
	print("Query 2:- {} ---> {}".format(query, nb_clf.predict(query)))

	#Query 3:
	query = np.array([['slonecznie', 'tak', 'srednia', 'wysoka']])
